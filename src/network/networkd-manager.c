...
int manager_connect_bus(Manager *m) {
        ...
        r = sd_bus_add_fallback_vtable(m->bus, NULL, "/org/freedesktop/network1/link", bus_network_cmpnt("Link"), link_vtable, link_object_find, m);
        if (r < 0)
               return log_error_errno(r, "Failed to add link object vtable: %m");

        r = sd_bus_add_fallback_vtable(m->bus, NULL, "/org/freedesktop/network1/link", bus_network_cmpnt("DHCPServer"), dhcp_server_vtable, link_object_find, m);
        if (r < 0)
               return log_error_errno(r, "Failed to add link object vtable: %m");
        ...
        r = sd_bus_add_fallback_vtable(m->bus, NULL, "/org/freedesktop/network1/network", bus_network_cmpnt("Network"), network_vtable, network_object_find, m);
        if (r < 0)
        ...
        r = sd_bus_request_name_async(m->bus, NULL, bus_network_mgr->destination, 0, NULL, NULL);
        if (r < 0)
